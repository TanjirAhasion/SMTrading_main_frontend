...
import { RentalActiveReportRow, SerialReportService } from '../../../service/reports/serial-report.service';
...
@Component({
  selector: 'app-rental-active-report',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ReportTableComponent],
  templateUrl: './rental-active-report.html'
})
export class RentalActiveReportComponent implements OnInit {
  private fb = inject(FormBuilder);
  filterForm = createReportFilterForm(this.fb);
  products: Product[] = [];
  brands: Brand[] = [];
  customers: Customer[] = [];
  rows: RentalActiveReportRow[] = [];
  totalCount = 0;
  page = 1;
  pageSize = 20;
  ...
  columns: ReportTableColumn[] = [
    ...
    { key: 'rentalStartDate', label: 'Start Date', type: 'date', sortable: true },
    { key: 'expectedReturnDate', label: 'Expected Return', type: 'date' },
    { key: 'status', label: 'Status', type: 'status' },
    { key: 'isOverdue', label: 'Overdue', type: 'boolean', align: 'center' }
  ];
  ...
  loadReport(): void {
    this.loading = true;
    this.reports.getRentalActive(compactFilter(this.filterForm, this.page, this.pageSize, this.sortBy, this.sortDirection)).subscribe({
  ...
  export(type: 'excel' | 'pdf'): void { if (this.rows.length) exportReport(this.excel, this.pdf, this.rows, this.columns, 'Active Rental Report', 'active-rental-report', type); }
